...
class WP_REST_Shortcodes_Controller_bpps extends WP_REST_Controller {
	...
	public function get_shortcode_output_permissions_check( $request ) {
		if ( ! current_user_can( 'edit_posts' ) ) {
			return new WP_Error( 'gutenberg_shortcode_block_cannot_read', esc_attr__( 'Sorry, you are not allowed to read shortcode blocks as this user.', 'gutenberg' ), array(
	...
	public function get_shortcode_output( $request ) {
		...
		if ( empty( $shortcode ) ) {
			$data['html'] = esc_attr__( 'Enter something to preview', 'gutenberg' );
		...
		if ( empty( $output ) || ( $output === $shortcode ) ) {
			$data['html'] = esc_attr__( 'Sorry, couldn\'t render a preview', 'gutenberg' );
			return rest_ensure_response( $data );
		}
	...
	public function get_item_schema() {
		return array(
			...
			'properties' => array(
				'html'  => array(
					'description' => esc_attr__( 'The block\'s content with shortcodes filtered through hooks.', 'gutenberg' ),
					'type'        => 'string',
					'required'    => true,
				),
				'type'  => array(
					'description' => esc_attr__( 'The filtered content type - video or otherwise', 'gutenberg' ),
					'type'        => 'string',
					'required'    => true,
				),
				'style' => array(
					'description' => esc_attr__( 'Links to external style sheets needed to render the shortcode', 'gutenberg' ),
					'type'        => 'string',
					'required'    => true,
				),
				'js'    => array(
					'description' => esc_attr__( 'Links to external javascript and inline scripts needed to render the shortcode', 'gutenberg' ),
					'type'        => 'string',
					'required'    => true,
				),
			),
